...
const Contact = () => {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      unread: { color: 'bg-red-100 text-red-800', icon: FiMailIcon, label: 'Unread' },
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-red-500">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center">
              <FiMailIcon className="w-6 h-6 text-red-600" />
            </div>
          </div>
        </div>
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredMessages.map((message) => (
                  <tr key={message.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setSelectedMessage(message)}>
